...
$js					= "jQuery(document).ready(function() {
						...
						function() {
							if (jQuery('ul',jQuery(this).parent().parent()).is(':hidden')) {
								jQuery('ul',jQuery(this).parent().parent()).slideDown(500,'linear').parent('li').addClass('VmOpen').removeClass('VmClose');
							} else {
...
if(!function_exists('vm_template_get_tree_recurse')){
	function vm_template_get_tree_recurse($category,$childs,$parentCategories,$vendorId,$class_sfx,$ID,$level = 0){
		...
		if(is_array($childs) && sizeof($childs)):
			...
			<ul class="menu<?php echo $class_sfx; ?> vm-catelgries-level-<?php echo $level; ?>">
					<?php
					foreach ($childs as $child) {
						
						$active_menu = 'VmClose';
						if (in_array( $child->virtuemart_category_id, $parentCategories)) $active_menu = 'VmOpen';
						...
						<li id="module-<?php echo $ID; ?>-vm-category-<?php echo $child->virtuemart_category_id; ?>" class="vm-category-<?php echo $child->virtuemart_category_id; ?> <?php echo $active_menu ?>">
							<div class="vm-category-title<?php if (is_array($child->childs) && sizeof($child->childs)):?> has-children<?php endif; ?>">
...
<ul class="VMmenu<?php echo $class_sfx ?> vm-catelgries-level-0" id="<?php echo "VMmenu".$ID ?>" >
<?php foreach ($categories as $category) {
		...
		?>
	<li id="module-<?php echo $ID; ?>-vm-category-<?php echo $category->virtuemart_category_id; ?>" class="vm-category-<?php echo $category->virtuemart_category_id; ?> <?php echo $active_menu ?>">
		<div class="vm-category-title<?php if (is_array($category->childs) && sizeof($category->childs)):?> has-children<?php endif; ?>">
			<?php echo JHTML::link($caturl, $cattext);
