...
// from name
	...
	echo "\n".'<div class="fc_line"><label class="fc_left">'.JText::_('COM_FLEXICONTACT_FROM_NAME').'</label>';
	echo "\n".'<span><input type="text" class="fc_input" name="from_name" value="'.$this->escape($this->post_data->from_name).'" />'.
				self::get_error('from_name').'</span>';
	echo "\n".'</div>';

// from email address

	echo "\n".'<div class="fc_line"><label class="fc_left">'.JText::_('COM_FLEXICONTACT_FROM_ADDRESS').'</label>';
	echo "\n".'<span><input type="text" class="fc_input" name="from_email" value="'.$this->escape($this->post_data->from_email).'" />'.
...
// subject
	...
	if ($this->config_data->show_subject)
		{
		echo "\n".'<div class="fc_line"><label class="fc_left">'.JText::_('COM_FLEXICONTACT_SUBJECT').'</label>';
		echo "\n".'<span><input type="text" class="fc_input" name="subject" value="'.$this->escape($this->post_data->subject).'" />'.
...
// the select list
	...
	if ($this->config_data->list_opt != 'disabled')
		...
		echo "\n".'<div class="fc_line"><label class="fc_left">'.$this->config_data->list_prompt.'</label>';
...
// the five optional fields
	...
	for ($i=1; $i<=5; $i++)
		...
		echo "\n".'<div class="fc_line"><label class="fc_left">'.$this->config_data->$prompt_name.'</label>';
		echo "\n".'<span><input type="text" class="fc_input" name="'.$field_name.'" value="'.$this->escape($this->post_data->$field_name).'" />'.
...
// the main text area
	...
	if ($this->config_data->area_opt != 'disabled')
		...
		echo "\n".'<div class="fc_line"><label class="fc_left">'.$this->config_data->area_prompt.'</label>';
		echo "\n".'<span><textarea class="fc_input" name="area_data" rows="'.$this->config_data->area_height.'" '.$size.'>'.$this->escape($this->post_data->area_data).'</textarea>'.
...
// the magic word
	...
	if ($this->config_data->magic_word != '')
		{
		echo "\n".'<div class="fc_line"><label class="fc_left">'.JText::_('COM_FLEXICONTACT_MAGIC_WORD').'</label>';
		echo "\n".'<span><input type="text" class="fc_input" name="magic_word" value="'.$this->escape($this->post_data->magic_word).'" />'.
